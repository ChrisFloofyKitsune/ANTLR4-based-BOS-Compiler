# Copied from .moderngl_window/loaders/scene/gltf2.py
...
import gltf.pydantic_models.material
import gltf.pydantic_models.texture
...
class GLTFAnimLoader(BaseLoader):
    ...
    def __init__(self, meta: SceneDescription):
        ...
        self.images: list[gltf.pydantic_models.material.Texture] = []
        self.samplers: list[gltf.pydantic_models.material.Sampler] = []
        self.textures: list[MaterialTexture] = []

        self.path: Optional[Path] = None
        self.scene: Scene | None = None
        self.gltf: GLTFMeta | None = None
...
class GLTFMeta:
    ...
    def __init__(
        ...
        """
        :param path: GLTF file name loaded
        ...
        self.animations = [GLTFAnimation(a) for a in data.get("animations", [])]
        self.asset = GLTFAsset(data["asset"])
        self.materials = [GLTFMaterial(m) for m in data.get("materials", [])]
        self.images = [GLTFImage(i) for i in data.get("images", [])]
        self.samplers = [GLTFSampler(s) for s in data.get("samplers", [])]
        self.textures = [GLTFTexture(t) for t in data.get("textures", [])]
        self.scenes = [GLTFScene(s) for s in data.get("scenes", [])]
        self.nodes = [GLTFNode(n) for n in data.get("nodes", [])]
        self.meshes = [GLTFMesh(m, self.meta) for m in data.get("meshes", [])]
        self.cameras = [GLTFCamera(c) for c in data.get("cameras", [])]

        self.buffer_views = [GLTFBufferView(i, v) for i, v in enumerate(data.get("bufferViews", []))]
        self.buffers = [GLTFBuffer(i, b, self.path.parent) for i, b in enumerate(data.get("buffers", []))]
        self.accessors = [GLTFAccessor(i, a) for i, a in enumerate(data.get("accessors", []))]
    ...
    def _link_data(self) -> None:
        ...
        # animations
        #   channels -> samplers + nodes
        #   samplers -> accessors
        for anim in self.animations:
            for channel in anim.channels:
                channel.sampler = anim.samplers[channel.samplerId]
                channel.target.node = self.nodes[channel.target.nodeId]

            for sampler in anim.samplers:
                sampler.input_ = self.accessors[sampler.inputId]
                sampler.output = self.accessors[sampler.outputId]
    ...
    def check_extensions(self, supported: list[str]) -> None:
        ...
        """
        ext_req = self.data.get("extensionsRequired")
        if ext_req is not None:
            for ext in ext_req:
                if ext not in supported:
                    raise ValueError(f"Extension '{ext}' not supported")
        ext_use = self.data.get("extensionsUsed")
        if ext_use is not None:
            for ext in ext_use:
                if ext not in supported:
                    raise ValueError(f"Extension '{ext}' not supported")
...
class GLTFAnimation:
    class Channel:
        class Target:
            ...
            def __init__(self, data: dict[str, Any]):
                self.nodeId: int = data.get("node", 0)
                self.node: GLTFNode | None = None
                self.path: str = GLTFAnimation.Channel.Target.Path(data.get("path"))
                self.extensions = data.get("extensions", {})

        def __init__(self, data: dict[str, Any]):
            self.samplerId: int = data.get("sampler")
            self.sampler: GLTFAnimation.Sampler | None = None
            self.target = GLTFAnimation.Channel.Target(data.get("target"))
            self.extensions = data.get("extensions", {})
    ...
    class Sampler:
        ...
        def __init__(self, data: dict[str, Any]):
            self.inputId: int = data.get("input")
            self.input_: GLTFAccessor | None = None

            self.interpolation = GLTFAnimation.Sampler.Interpolation(
                data.get("interpolation", GLTFAnimation.Sampler.Interpolation.Linear)
            )

            self.outputId: int = data.get("output")
            self.output: GLTFAccessor | None = None
            self.extensions = data.get("extensions", {})

    def __init__(self, data: dict[str, Any]):
        self.channels: list[GLTFAnimation.Channel] = [
            GLTFAnimation.Channel(c) for c in data.get("channels")
        ]
        self.samplers: list[GLTFAnimation.Sampler] = [
            GLTFAnimation.Sampler(s) for s in data.get("samplers")
        ]
...
class GLTFMesh:
    class Primitives:
        def __init__(self, data: dict[str, Any]):
            ...
            self.extensions = data.get("extensions", {})
            self.accessor: GLTFAccessor | None = None

        def load_indices(self) -> tuple[ComponentType, npt.NDArray[Any]] | tuple[None, None]:
            """Loads the index buffer / polygon list for a primitive"""
            if self.indices is None or self.accessor is None:
                return None, None

            _, component_type, buffer = self.accessor.read()
            return component_type, buffer

        def prepare_attrib_mapping(self, ) -> list[VBOInfo]:
            """Pre-parse buffer mappings for each VBO to detect interleaved data for a primitive"""
            buffer_info: list[VBOInfo] = []
            for name, accessor in self.attributes.items():
        ...
        def get_bbox(self) -> tuple[glm.vec3, glm.vec3]:
            """Get the bounding box for the mesh"""
            accessor = self.attributes.get("POSITION")
    ...
    def load(self, materials: list[Material]) -> list[Mesh]:
        ...
        for primitive in self.primitives:
            ...
            # Index buffer
            component_type, index_vbo = primitive.load_indices()
            ...
            attributes = {}
            vbo_infos = primitive.prepare_attrib_mapping()

            for vbo_info in vbo_infos:
            ...
            bbox_min, bbox_max = primitive.get_bbox()
            meshes.append(
                Mesh(
                    self.name,
                    vao=vao,
...
class GLTFAccessor:
    def __init__(self, accessor_id: int, data: dict[str, Any]):
        ...
        self.bufferView: GLTFBufferView | None = None
...
class GLTFBufferView:
    def __init__(self, view_id: int, data: dict[str, Any]):
        ...
        self.buffer: GLTFBuffer | None = None
        self.byteOffset = data.get("byteOffset", 0)
...
class GLTFImage:
    ...
    Image may be a file, embedded data or pointer to data in bufferview
    """
    ...
    def load(self, path: Path) -> gltf.pydantic_models.material.Texture:
